export default {
  ...
  max_conn_rate: {
    desc: '用于限制单节点接受连接的速率，达到限制后，EMQX 将暂停从接受队列中获取连接，延迟或拒绝新的连接；为空时表示不限制。\n\n例如：\n\n- `1000/s` 表示每秒最多接受 1000 个连接。\n- `1000/10s` 表示每 10 秒最多接受 1000 个连接。',
    label: '最大连接速率',
  },
  max_conn_burst: {
    desc: '在突发情况下单节点可以接受的最大连接数量；为空时表示不限制。\n\n例如：`10000/60m` 表示在 60 分钟内，允许接受最多 10000 个连接。',
    label: '最大连接突发速率',
  },
  messages_rate: {
    desc: '用于限制单个客户端每秒可以发送给 EMQX 的消息数量，达到限制后，EMQX 将暂停从接收缓冲区读取数据，降低发送者的速度或暂时挂起发送者；为空时表示不限制。\n\n例如：\n\n- `500/s`：每秒最多接收 500 条消息，其余消息将被延迟。\n- `500/10s`：每 10 秒最多接收 500 条消息，其余消息将被延迟。',
    label: '消息发布速率',
  },
  messages_burst: {
    desc: '在突发情况下单个客户端可以发送的最大消息数量；为空时表示不限制。\n\n例如：`10000/60m` 表示在 60 分钟内，允许发送最多 10000 条消息。',
    label: '消息发布突发速率',
  },
  bytes_rate: {
    desc: '用于限制单个客户端每秒可发送给 EMQX 的字节数，达到限制后，EMQX 将暂停从接收缓冲区读取数据，降低发送者的速度或暂时挂起发送者；可设置为数字或 `100MB/60m` 格式的字符串，为空时表示不限制。\n\n字节的单位可以是：B、KB、MB、GB。\n\n例如：\n\n- `500KB/s`：每秒最多接收 500 千字节，其余字节将被延迟。\n- `500MB/10s`：每 10 秒最多接收 500 兆字节，其余字节将被延迟。',
    label: '数据发布速率',
  },
  bytes_burst: {
    desc: '在突发情况下单个客户端允许发送的最大字节数；可设置为数字或 `100MB/60m` 格式的字符串，为空时表示不限制。\n\n例如：`100MB/60m` 表示在 60 分钟内，允许发送最多 100 兆字节的数据。',
    label: '数据发布突发速率',
  },
  /* MQTT END */
  /* SESSION START */
