...
一旦达到限制，EMQX 将自动关闭任何新连接，这些连接在被接受后会立即被关闭，从而有效地清空等待被接受的连接队列。

例如：

- \`1000/s\`: 每秒最多接受 1000 个连接。
- \`1000/10s\`: 每 10 秒最多接受 1000 个连接。`,
    label: '最大连接速率',
  },
  max_conn_burst: {
    desc: `每个节点在常规速率基础上可突发接受的连接数。

例如：\`10000/60m\`: 每 60 分钟允许在短时间内突发接受最多 10000 个连接。`,
    label: '最大连接突发速率',
  },
  messages_rate: {
...
- \`500/s\`: 每秒最多接收 500 条消息，超出部分将被丢弃/拒绝。
- \`500/10s\`: 每 10 秒最多接收 500 条消息，超出部分将被丢弃/拒绝。`,
    label: '消息发布速率',
  },
  messages_burst: {
    desc: `在常规 \`消息发布速率\` 基础上，每个节点允许突发发送的消息数量。
...
- \`500KB/s\`: 每秒最多接收 500 千字节，超出部分将被丢弃/拒绝。
- \`500MB/10s\`: 每 10 秒最多接收 500 兆字节，超出部分将被丢弃/拒绝。`,
    label: '报文发布速率',
  },
  bytes_burst: {
    desc: `在常规 \`报文发布速率\` 基础上允许突发发送的数据量。

例如：\`100MB/60m\`: 每 60 分钟允许在短时间内突发发送最多 100 兆字节。`,
    label: '报文发布突发速率',
  },
  delivery_messages_rate: {
    desc: `限制 EMQX 向单个客户端投递消息的速率（每个节点）。注意此配置方向与 \`消息发布速率\` 相反。

仅在使用内存会话时生效（即 \`durable_sessions.enable = false\`，会话持久化功能关闭）。

达到限制后，EMQX 将丢弃 QoS 0 消息，QoS 1 和 QoS 2 消息将被加入客户端内部队列并在稍后重试。

例如：

- \`500/s\`: 每秒最多投递 500 条消息，超出部分将被丢弃/排队。
- \`500/10s\`: 每 10 秒最多投递 500 条消息，超出部分将被丢弃/排队。`,
    label: '消息投递速率',
  },
  delivery_messages_burst: {
    desc: `在常规 \`消息投递速率\` 基础上，每个节点允许突发投递的消息数量。

仅在使用内存会话时生效（即 \`durable_sessions.enable = false\`，会话持久化功能关闭）。

例如：\`10000/60m\`: 每 60 分钟允许在短时间内突发投递最多 10000 条消息。`,
    label: '消息投递突发速率',
  },
  delivery_bytes_rate: {
    desc: `限制 EMQX 向单个客户端投递数据的速率（每个节点）。注意此配置方向与 \`报文发布速率\` 相反。
...
- \`500KB/s\`: 每秒最多投递 500 千字节，超出部分将被丢弃/排队。
- \`500MB/10s\`: 每 10 秒最多投递 500 兆字节，超出部分将被丢弃/排队。`,
    label: '数据投递速率',
  },
  delivery_bytes_burst: {
    desc: `在常规 \`数据投递速率\` 基础上允许突发投递的数据量。

仅在使用内存会话时生效（即 \`durable_sessions.enable = false\`，会话持久化功能关闭）。

例如：\`100MB/60m\`: 每 60 分钟允许在短时间内突发投递最多 100 兆字节。`,
    label: '数据投递突发速率',
  },
  /* MQTT END */
  /* SESSION START */
  ...
  durable_sessions_shared_subs_checkpoint_interval: {
    label: '检查间隔',
    desc: '共享订阅领导者定期将主题重放进度提交到数据库。此参数指定此类提交之间的最小时间间隔。',
  },
  realloc_interval: {
    label: '重新分配间隔',
    desc: `共享订阅领导者在此时间段内累积对组的更改（例如订阅者加入或离开组），然后在订阅者之间重新分配工作。

这样做是为了避免在短时间内多个订阅者加入或离开组时产生不必要的工作。`,
  },
  revocation_timeout: {
    label: '撤销超时',
    desc: `给共享订阅者完成最后一批消息处理的最大时间。

共享订阅组的重新平衡可能涉及将工作从一个订阅者转移到另一个订阅者。
第一个订阅者可以在此间隔内完成被撤销任务的剩余工作。

如果订阅者未能及时回复，它将被踢出组，其所有工作将被重新分配。`,
  },
