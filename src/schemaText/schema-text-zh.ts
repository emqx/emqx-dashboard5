export default {
  ...
  messages_rate: {
    desc: '用于限制单个客户端每秒可以发送给 EMQX 的消息数量。\n\n一旦达到限制，EMQX 将丢弃 QoS 0 消息，并使用"配额超限"错误码（0x97）拒绝 QoS 1 和 QoS 2 消息。\n\n例如：\n- `500/s`：每秒最多接收 500 条消息，其余消息将被丢弃/拒绝。\n- `500/10s`：每 10 秒最多接收 500 条消息，其余消息将被丢弃/拒绝。',
    label: '消息发布速率',
  },
  messages_burst: {
    desc: '允许在突发情况下额外发送的消息数量。\n\n例如：`10000/60m`：在 60 分钟内，允许一次性在短时间内发送最多 10000 条消息。',
    label: '消息发布突发速率',
  },
  bytes_rate: {
    desc: '用于限制单个客户端每秒可发送给 EMQX 的字节数。\n\n一旦达到限制，EMQX 将丢弃 QoS 0 消息，并使用"配额超限"错误码（0x97）拒绝 QoS 1 和 QoS 2 消息。\n\n字节的单位可以是：B、KB、MB、GB。\n\n例如：\n- `500KB/s`：每秒最多接收 500KB，其余字节将被丢弃/拒绝。\n- `500MB/10s`：每 10 秒最多接收 500MB，其余字节将被丢弃/拒绝。',
    label: '数据发布速率',
  },
  bytes_burst: {
    desc: '允许在突发情况下额外发送的字节数。\n\n例如：`100MB/60m`：在 60 分钟内，允许一次性在短时间内发送最多 100MB 的数据。',
    label: '数据发布突发速率',
  },
  /* MQTT END */
  /* SESSION START */
