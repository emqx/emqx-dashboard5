...
Please note that each message should have the property set by "Stream Key Expression" to be saved in the stream.`,
  ...
  keyExpressionDesc: {
    zh: `用于确定消息流中消息的流键表达式，EMQX 将流键相同的消息放入同一个分区，从而保证消息的顺序性。<br />默认值为 \`message.from\`，即为消息的发送者客户端 ID，关于 variform 表达式可使用的函数及语法请参考 [Variform 表达式](https://docs.emqx.com/zh/emqx/latest/configuration/configuration.html#variform-%E8%A1%A8%E8%BE%BE%E5%BC%8F) 章节。注意：不支持从 \`message.payload.xxx\` 中取值作为流键<br />
可使用的变量请参考以下数据结构：
`,
    en: `The expression used to determine the key of the message for Message Streams, EMQX will put messages with the same stream key into the same partition to ensure the order of messages.<br />The default value is \`message.from\`, which is the client ID of the message publisher. For functions and syntax of variform expression, please refer to the [Variform Expressions](https://docs.emqx.com/en/emqx/latest/configuration/configuration.html#variform-expressions) chapter. Note: Extracting values from \`message.payload.xxx\` is not supported as stream key.
