...
Please note that each message should have the property set by "Queue Key Expression" to be saved in the queue.`,
  ...
  keyExpressionDesc: {
    zh: '用于确定 Last-Value 消息队列中消息的队列键表达式，默认值为 `message.from`，即为消息的发送者客户端 ID，关于 variform 表达式的更多信息请参考 [Variform 表达式](https://docs.emqx.com/zh/emqx/latest/configuration/configuration.html#variform-%E8%A1%A8%E8%BE%BE%E5%BC%8F) 章节',
    en: 'The expression used to determine the key of the message for Last-Value Message Queues, the default value is `message.from`, which is the client ID of the message publisher. For more information about the variform expression, please refer to the [Variform Expressions](https://docs.emqx.com/en/emqx/latest/configuration/configuration.html#variform-expressions) chapter.',
  },
