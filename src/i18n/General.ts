...
- **角色默认权限**：自动使用当前角色的默认权限；切换角色或以后角色默认权限发生变化时，权限会自动更新。
...
- **Role Default Scopes**: Automatically use the current role's defaults, including when the role or its defaults change.
