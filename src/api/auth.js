...
export function moveAuthz(type, body) {
  return http.post(
    `/authorization/sources/${encodeURIComponent(type)}/move`,
    body
  );
}
...
export function deleteAuthnUser(id, userId) {
  return http.delete(
    `/authentication/${encodeURIComponent(id)}/users/${encodeURIComponent(
      userId
    )}`
  );
}
...
export function deleteBuiltInDatabaseData(type, key) {
  return http.delete(
    `/authorization/sources/built-in-database/${type}/${encodeURIComponent(
      key
    )}`
  );
}
