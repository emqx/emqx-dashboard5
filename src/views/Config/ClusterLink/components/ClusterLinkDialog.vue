            ...
            <ul class="topic-list">
              <li class="topic-item" v-for="(item, $index) in record.topics" :key="$index">
                <el-form-item :prop="`topics.${$index}`" :rules="arrayItemRule.topic">
                  <el-input v-model="record.topics[$index]" />
                  <div class="btn-container vertical-align-center">
                    ...
                    >
                      <el-button
                        ...
                        :icon="Minus"
                        :disabled="
                          record.topics.length <= 1 || record.enable || !$hasPermission('delete')
                        "
...
const generateRawRecord = (): ClusterLink => ({
  enable: true,
  name: '',
  server: '',
  clientid: '',
...
const { addLink } = useClusterLink()
const save = async () => {
  try {
    await FormCom.value.validate()
    const data = { ...record.value, ssl: handleSSLDataBeforeSubmit(record.value.ssl as any) as any }
