  ...
  <div class="database-config config">
    ...
    >
      <div class="config-sub-block">
        <el-form-item v-if="isRedis" :label="$t('Auth.redisType')" prop="redis_type" required>
        ...
        <el-form-item v-if="isServers" :label="$t('Auth.servers')" required prop="servers">
          <el-input v-model="databaseConfig.servers" type="textarea" :rows="3" />
        </el-form-item>

        <el-form-item v-else :label="$t('Auth.server')" required prop="server">
          <el-input v-model="databaseConfig.server" />
        </el-form-item>

        <el-form-item
          v-if="isMongoDB && databaseConfig.mongo_type === MongoType.Rs"
          :label="$t('Auth.replicaSetName')"
          required
          prop="replica_set_name"
        >
          <el-input v-model="databaseConfig.replica_set_name" />
        </el-form-item>
        <!-- Redis -->
        <el-form-item
          v-if="isRedis && databaseConfig.redis_type === 'sentinel'"
          :label="$t('Auth.sentinel')"
          prop="sentinel"
          required
        >
        ...
        <!-- Basic -->
        <el-form-item
          v-if="!(isRedis && databaseConfig.redis_type === 'cluster')"
        ...
        <el-form-item v-if="isMongoDB" :label="$t('Auth.collection')" required prop="collection">
          <el-input v-model="databaseConfig.collection" />
        </el-form-item>

        <el-form-item v-if="!isRedis" :label="$t('Base.username')">
        ...
        <el-form-item v-if="isAuthn && isMongoDB" :label="$t('Auth.passwordHashField')">
        ...
        <template v-if="isMongoDB">
          <el-form-item v-if="isAuthn && isEnableSalt" :label="$t('Auth.saltField')">
          ...
          <template v-if="databaseConfig.mongo_type === 'rs'">
            ...
            <el-form-item v-if="databaseConfig.mongo_type === 'rs'" :label="$t('Auth.writeMode')">
          ...
          <el-form-item v-if="isAuthn" :label="$t('Auth.superuserField')">
            <el-input
      ...
      <div class="config-sub-block">
        ...
        <template v-else-if="isRedis">
          <el-form-item required prop="cmd" class="label-whole-line">
            ...
            <el-button class="button-in-label-line" size="small" @click="setDefaultContent('cmd')">
              {{ $t('Auth.setDefault') }}
            </el-button>
            <el-button class="help-btn" size="small" @click="toggleNeedHelp">
              {{ $t('Base.help') }}
        ...
        <AdvancedSettingContainer>
          ...
          <el-form-item v-if="isAuthn && isMySQL" :label="$t('Auth.queryTimeout')">
            <time-input-with-unit-select v-model="databaseConfig.query_timeout" />
          </el-form-item>

          <el-form-item v-if="isMongoDB" :label="$t('Auth.connectTimeout')">
            <time-input-with-unit-select v-model="databaseConfig.topology.connect_timeout_ms" />
          </el-form-item>

          <el-form-item v-if="isPgSQL" prop="disable_prepared_statements">
            <template #label>
              <FormItemLabel
